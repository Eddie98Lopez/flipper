...
exports.up = async function(knex) {
    await knex.schema.createTable('contacts',(contacts)=>{
        ...
        contacts.integer('home_id').unsigned().notNullable().references('home_id').inTable('homes').onDelete('CASCADE')
    ...
    await knex.schema.createTable('contact_log',(cLog)=>{
        ...
        cLog.integer('home_id').unsigned().notNullable().references('home_id').inTable('homes').onDelete('RESTRICT')
        cLog.integer('contact_id').unsigned().notNullable().references('contact_id').inTable('contacts').onDelete('RESTRICT')
        cLog.integer('user_id').unsigned().notNullable().references('user_id').inTable('users').onDelete('RESTRICT')
